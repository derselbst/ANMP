...
void BlockAnalyzer::initializeGL()
{
    // Disable depth test (all is drawn on a 2d plane)
    glDisable(GL_DEPTH_TEST);
}

void BlockAnalyzer::resizeGL(int w, int h)
{
...
void BlockAnalyzer::paintGL()
{
    ...
    for (uint y, x = 0; x < (uint)m_scope.size(); ++x)
    {
        ...
        // Draw top bar
        drawTexture(m_topBarTexture.data(), x * (BLOCK_WIDTH + 1), int(m_store[x]) * (BLOCK_HEIGHT + 1), 0, 0);
    }
}

void BlockAnalyzer::drawTexture(Texture *texture, int x, int y, int sx, int sy)
{
    const GLfloat xf = x;
    const GLfloat yf = y;
