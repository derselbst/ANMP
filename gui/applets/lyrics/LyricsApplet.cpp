...
class LyricsAppletPrivate
{
    ...
    : saveIcon(0), editIcon(0), autoScrollIcon(0), reloadIcon(0), closeIcon(0), settingsIcon(0), browser(0), suggestView(0), currentTrack(0), alignment(Qt::AlignLeft), hasLyrics(false), showBrowser(false), showSuggestions(false), isShowingUnsavedWarning(false), userAutoScrollOffset(0), oldSliderPosition(0), q_ptr(parent)
    {
    }
    ~LyricsAppletPrivate()
    {
...
void LyricsAppletPrivate::setEditing(bool isEditing)
{
    browser->setReadOnly(!isEditing);
}

void LyricsAppletPrivate::determineActionIconsState()
{
...
void LyricsAppletPrivate::showLyrics(const QString &text)
{
    browser->clear();
...
void LyricsAppletPrivate::showSuggested(const QVariantList &suggestions)
{
...
void LyricsAppletPrivate::refetchLyrics()
{
    DEBUG_BLOCK
...
void LyricsAppletPrivate::showUnsavedChangesWarning(Meta::TrackPtr newTrack)
{
    Q_Q(LyricsApplet);
...
void LyricsAppletPrivate::_changeLyricsAlignment()
{
...
void LyricsAppletPrivate::_changeLyricsFont()
{
...
void LyricsAppletPrivate::_editLyrics()
{
    if (!hasLyrics)
    {
        browser->clear();
...
void LyricsAppletPrivate::_closeLyrics()
{
    if (hasLyrics)
    {
...
void LyricsAppletPrivate::_saveLyrics()
{
    if (currentTrack)
    {
...
void LyricsAppletPrivate::_toggleAutoScroll()
{
    Q_Q(LyricsApplet);
...
void LyricsAppletPrivate::_suggestionChosen(const LyricsSuggestion &suggestion)
{
    DEBUG_BLOCK
...
void LyricsAppletPrivate::_unsetCursor()
{
...
void LyricsAppletPrivate::_trackDataChanged(Meta::TrackPtr track)
{
...
void LyricsAppletPrivate::_trackPositionChanged(qint64 position, bool userSeek)
{
    Q_UNUSED(userSeek);
...
: Context::Applet(parent, args), d_ptr(new LyricsAppletPrivate(this))
{
...
void LyricsApplet::createConfigurationInterface(KConfigDialog *parent)
{
    Q_D(LyricsApplet);
...
void LyricsApplet::keyPressEvent(QKeyEvent *e)
{
    Q_D(LyricsApplet);
